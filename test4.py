...
model.add(keras.layers.GRU(128, return_sequences=True, input_shape=(16, 9), activation='relu'))
model.add(keras.layers.GRU(64, input_shape=(16, 9), activation='relu'))
...
EPOCHS = 2
model.fit(w.train, epochs=EPOCHS, validation_data=w.val)

logits = model.predict(w.test)
predictions = tf.nn.softmax(logits)
predicted_classes = np.argmax(predictions, 1)

actual_values = []
for x, y in w.test.unbatch():
    actual_values.append(y.numpy()[0][0])

diff = actual_values - predicted_classes
unique, counts = np.unique(diff, return_counts=True)
occurences = dict(zip(unique, counts))
right = occurences[0]
wrong = len(diff) - right

loss, accuracy = model.evaluate(w.test)
# fix error:
# https://github.com/tensorflow/tensorflow/issues/47691#issuecomment-889499511
